...
# ---------------------------------------------------------------------------
# Integrated StreamExtract v1.13 + TempOverlay
# ---------------------------------------------------------------------------
...
# StreamExtract v1.13 makes ZIP Range parallelism user-selectable (1-3 HTTP
# connections). One connection keeps the continuous streaming reader and gives
# the smoothest Current file progress; 2-3 connections may improve throughput on
# servers that limit each connection, while thermal protection still caps the
# aggregate feed rate across all connections.
...
class DownloadWorker(QObject):
    ...
    def _read_process_output(self, pipe, label: str, tail: list[str], stats: dict | None = None):
        if pipe is None:
            return
        try:
            for raw in iter(pipe.readline, b""):
                ...
                # bsdtar -v generally prints one archive member per line.
                # Strip a leading "x " if this build includes it.
                display = line[2:] if line.startswith("x ") else line

                # Never present extractor diagnostics as if they were filenames.
                # Some SteamOS libarchive/RAR5 builds emit `bsdtar: (null)` at
                # the exact end of an otherwise successful stdin extraction.
                lower = line.lower()
                is_tool_diagnostic = (
                    lower.startswith("bsdtar:")
                    or lower.startswith("7z:")
                    or lower.startswith("7-zip:")
                    or any(word in lower for word in ("error", "failed", "warning", "cannot", "seek error"))
                )
                ...
                if is_tool_diagnostic:
                    self.log.emit(line if lower.startswith(("bsdtar:", "7z:", "7-zip:")) else f"{label}: {line}")
                else:
    ...
    def _extract_with_bsdtar(self, archive_kind: str = "archive"):
        ...
        if archive_kind in ("RAR4", "RAR5"):
            with self._make_http_client() as metadata_client:
                remote_total = self._probe_remote_size(metadata_client)
                if remote_total > 0:
                    self._prepare_rar_final_size(metadata_client, archive_kind, remote_total)
        ...
        stdout_thread = threading.Thread(
            ...
            args=(proc.stdout, "bsdtar", tail, stats),
            daemon=True,
        )
        stderr_thread = threading.Thread(
            target=self._read_process_output,
            args=(proc.stderr, "bsdtar", tail, stats),
            daemon=True,
        )
        stdout_thread.start()
        stderr_thread.start()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        subtitle = QLabel(
            "Paste a direct ZIP or RAR link, choose storage, and extract directly into Moses OneClick."
        )
        ...
        self.url_edit.setPlaceholderText("https://example.com/archive.zip or archive.rar")
    ...
    def save_log(self):
        ...
        try:
            ...
            safe_url = f"{parsed.scheme}://{parsed.netloc}/[redacted]" if parsed.scheme and parsed.netloc else "—"
        except Exception:
            safe_url = "—"
    ...
    def validate_inputs(self):
        ...
        if parsed.scheme not in ("http", "https") or not parsed.netloc:
            QMessageBox.warning(
                self, "Invalid URL",
                "Please paste a direct HTTP or HTTPS download URL."
            )
            return None
        ...
        if self.performance_combo.currentText() == "Custom":
            try:
                ...
                return None
        return url, custom_rate, self.selected_range_connections()

    def start_download(self):
        validated = self.validate_inputs()
        if not validated:
            return
        url, custom_rate, range_connections = validated
    ...
    def on_finished(self):
        ...
        # Reuse Moses OneClick's normal Find Game EXE + Add to Steam chooser.
        # Files are already in the final <Game>/Files location while the stream
        # runs; finalization only chooses/validates the EXE, corrects the name if
        # requested, creates the Steam/Proton mapping, and starts artwork.
        ...
        if result.get("ok"):
            ...
            if result.get("shortcut_state") == "pending_steam_restart":
                shortcut_note = (
                    "\n\nSteam is currently open, so the shortcut will finalize on the next Steam restart "
                    "or when you return to Gaming Mode. Artwork is downloading in the background."
                )
            else:
                shortcut_note = "\n\nThe Steam shortcut is ready and artwork is downloading in the background."
            QMessageBox.information(
                self, "Game ready",
                f"{name} finished extracting and was added to Steam.{shortcut_note}"
            )
            return
    ...
    def on_failed(self, message: str):
        ...
        self.stream_session = None
        QMessageBox.critical(self, "StreamExtract error", message)
        self.worker = None
...
def add_existing_steam_exe(exe: Path, game_name: str):
    ...
    if commit.get("pending"):
        ...
        popup_text = f"{game_name} was added to Steam successfully. Artwork is downloading in the background."
    subprocess.run(
...
def _steam_main_process_running():
    """Return True only for Steam's main client process.

    A steamwebhelper can linger briefly after the client exits. Treating that
    helper as the Steam client made deferred shortcut writers miss the tiny
    shutdown window and is one reason Add Existing sometimes never appeared in
    Steam after a restart.
    """
    try:
        result = subprocess.run(
            ["pgrep", "-x", "steam"],
...
def stop_steam_cleanly():
    ...
    # On current SteamOS /usr/bin/steam is steam-jupiter and can add -pipewire.
    # Use the inner launcher for the shutdown half of a controlled shortcut
    # commit so we do not re-trigger KDE's Screen Sharing chooser.
    ...
    while time.time() < deadline:
        if not _steam_main_process_running():
            # Let Steam finish its final shortcuts.vdf/config writes before we
            # replace the file atomically with the OneClick shortcut included.
            time.sleep(0.35)
            return True
        time.sleep(0.15)
...
def _verify_steam_native_shortcut(appid):
    """Verify the exact non-Steam AppID is present in the active shortcuts.vdf."""
    try:
        return (int(appid) & 0xffffffff) in _steam_shortcut_appids()
    except Exception:
        return False


def _commit_steam_shortcut_reliably(appid, game_name, final_exe, icon_path=""):
    """Create now only when Steam is already closed; otherwise defer safely.

    Create Steam Shortcut must never stop or reopen Steam automatically. Steam
    can overwrite shortcuts.vdf while it is running, so the safe behavior is to
    queue the already-existing deferred finalizer and let the user restart/close
    Steam manually whenever they want. Artwork can still download immediately.
    """
    appid = int(appid)
    if steam_is_running():
        launch_deferred_steam_finalizer(appid)
        return {"ok": True, "pending": True, "restarted": False}

    try:
        if not _finalize_steam_shortcut_now(appid, game_name, Path(final_exe), icon_path):
            return {"ok": False, "error": "The Steam shortcut writer did not complete."}
        if not _verify_steam_native_shortcut(appid):
            time.sleep(0.15)
            _finalize_steam_shortcut_now(appid, game_name, Path(final_exe), icon_path)
            if not _verify_steam_native_shortcut(appid):
                return {"ok": False, "error": "The shortcut was written but could not be verified in Steam's active shortcuts.vdf."}
    except Exception as exc:
        return {"ok": False, "error": str(exc)}
...
class OneClickTools(Gtk.Window):
    def __init__(self):
        ...
        self.stream_extract_btn = Gtk.Button(label="StreamExtract")
        self.stream_extract_btn.set_hexpand(True)
        self.stream_extract_btn.set_size_request(-1, 42)
        self.stream_extract_btn.get_style_context().add_class("secondary")
        self.stream_extract_btn.set_tooltip_text("Open the integrated StreamExtract v1.13 downloader/extractor")
        self.stream_extract_btn.connect("clicked", self.on_stream_extract)
        utility_row.pack_start(self.stream_extract_btn, True, True, 0)

        self.temp_overlay_btn = Gtk.Button(label="TempOverlay")
        self.temp_overlay_btn.set_hexpand(True)
        self.temp_overlay_btn.set_size_request(-1, 42)
    ...
    def on_reselect_exe(self, _button):
        ...
        self.set_status(tr(f"Finding main EXE choices for {item['name']}…"))
        ...
        try:
            ...
            self.set_status(tr("Could not open main EXE chooser."))
            message(self, tr("Could not open main EXE chooser"), str(exc), Gtk.MessageType.ERROR)
            return

        if not payload.get("ok"):
            detail = str(payload.get("error") or "The game prefix could not be read.")
            self.set_status(tr("Could not open main EXE chooser."))
            message(self, tr("Could not open main EXE chooser"), detail, Gtk.MessageType.ERROR)
            return
        candidates = list(payload.get("candidates") or [])
        if not payload.get("available") or not candidates:
            self.set_status("")
            return

        game_name = str(payload.get("game_name") or item.get("name") or "Game")
        chooser = Gtk.Dialog(
            title=tr(f"Choose the main game EXE for {game_name}"),
            parent=self,
            flags=0,
        )
        chooser.set_default_size(620, 170)
        chooser.add_button(tr("Cancel"), Gtk.ResponseType.CANCEL)
        ok_button = chooser.add_button(tr("Use this EXE"), Gtk.ResponseType.OK)
        ok_button.get_style_context().add_class("suggested-action")

        area = chooser.get_content_area()
        area.set_spacing(10)
        area.set_border_width(16)
        label = Gtk.Label(label=tr("Choose the main game EXE:"))
        label.set_xalign(0.0)
        area.pack_start(label, False, False, 0)

        combo = Gtk.ComboBoxText()
        current = str(payload.get("current_exe") or "")
        active_index = 0
        ...
        try:
            result = subprocess.run(
                ["flatpak-spawn", "--host", str(helper), "reselect-exe-apply", str(appid), selected, "no-defer"],
                text=True,
                capture_output=True,
                timeout=45,
                check=False,
        ...
        exe_name = str(applied.get("final_exe_name") or Path(selected).name)
        if applied.get("pending_steam"):
            if self._offer_apply_pending_exe_now(appid):
                self.set_status(tr(f"Main EXE updated for {game_name}: {exe_name}."), success=True)
            else:
                self.set_status(tr(f"Main EXE updated for {game_name}: {exe_name}. Steam will refresh the shortcut when it next closes or changes mode."), success=True)
        else:
            self.set_status(tr(f"Main EXE updated for {game_name}: {exe_name}."), success=True)
...
echo "    - StreamExtract v1.13 integrated with Internal / External Moses storage

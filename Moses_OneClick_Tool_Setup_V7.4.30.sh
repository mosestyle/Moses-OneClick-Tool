...
# ---------------------------------------------------------------------------
# Integrated StreamExtract v1.22 + TempOverlay
# ---------------------------------------------------------------------------
...
# StreamExtract v1.22 keeps the reliable V1.21 multi-link/ISO pipeline and
# adds ordered multi-step follow-up update chains. If one follow-up source contains
# several incremental patch EXEs (for example 1.12.0 -> 1.16.0 -> 1.16.1 -> 1.17.0),
# Moses detects their version ranges, chooses the lowest valid step first, then
# opens the normal Game Installer for every next update in sequence. Steam shortcut
# finalization and artwork wait until the complete chain is finished. Ambiguous or
# disconnected update candidates are preserved instead of being run blindly.
# Non-update companion/DLC/redist sources are still preserved and never auto-run.
# Live multi-link concurrency remains adjustable from 1-10 while downloads are
# in progress. Single-link ZIP Range parallelism stays user-selectable (1-3 HTTP
...
def _batch_archive_layout(names: list[str]) -> dict:
    """Group a multi-link paste into one primary split set plus safe extras.

    V1.18 assumed every pasted URL belonged to one multipart archive. Real
    release pages often provide a split base game *and* one independent update
    archive in the same list. V1.20 recognizes that layout instead of trying to
    feed the update RAR to the multipart extractor as a fake third volume.

    For reliability we auto-install only when there is exactly one clear
    multipart set. Any unmatched ordinary archive becomes an independent
    follow-up source and is extracted separately so it can never overwrite the
    base install media.
    """
    if len(names) < 2:
    ...
    if len(multipart_groups) != 1:
        ...
        return {
            "ok": False,
            "error": (
                "More than one independent multipart archive set was detected. "
                "Please run those sets separately so Moses never guesses which one is the base installer."
            ),
        }
...
class MultiPartDownloadWorker(DownloadWorker):
    ...
    def run(self):
        try:
            ...
            if extras:
                self.log.emit(
                    f"Also detected {len(extras)} independent archive(s). They will be extracted separately as follow-up installer sources."
                )
                for extra in extras:
            ...
            if extras:
                ...
                for pos, extra in enumerate(extras, start=1):
                    ...
                    while out_dir.exists():
                        ...
                        suffix += 1
                    self._extract_local_archive(extra_entry, "Standalone archive", out_dir, 1)
                ...
                self.log.emit(
                    "Independent archive(s) were extracted under '_StreamExtract Follow-up' and marked so ISO cleanup will preserve them if they have not been used."
                )
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.keep_extracted_box.setToolTip(
            "Applies to the extracted installer source (for example the ISO and companion files). "
            "Temporary downloaded RAR/ZIP/7z inputs are always deleted after successful extraction so they do not duplicate storage. "
            "If unchecked, the used ISO/base installer source is deleted after a successful install and safe ISO unmount. "
            "Independent follow-up/update installer files that have not been used are preserved for safety. "
            "Installed game files are never deleted."
        )
    ...
    def set_running(self, running: bool):
        ...
        self.keep_extracted_box.setEnabled(not running)

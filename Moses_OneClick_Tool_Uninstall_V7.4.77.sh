...
GUI_RUNNER="$BIN_DIR/moses-oneclick-gui-python"
...
# Best-effort cleanup of any read-only ISO session created by StreamExtract.
# Busy installer discs are never force-unmounted; udisks will simply refuse.
if [[ -x "$HELPER" ]]; then
...
rm -rf "$HOME/.local/share/moses-oneclick-runtime"
remove_moses_tempoverlay_kwin_rule
restore_iso_handlers
rm -rf "$DATA_DIR"
rm -rf "$HOME/.cache/lutris-exe-helper"
rm -rf "$HOME/.local/share/oneclick-exe/runtime-cache"
rm -rf "$HOME/.local/share/oneclick-exe/game-folders"
